...
class MrpProductionInherit(models.Model):
    ...
    check_request= fields.Boolean(string="Generar Request", default=False)
    user_id = fields.Many2one('res.users', 'Usuario', track_visibility='onchange', default=_get_default_user_id)
...
###START ADDED
        for x in self.move_raw_ids.filtered(lambda mv: mv.product_id.purchase_ok and mv.state in ('confirmed')  and (mv.quantity_available < mv.product_uom_qty)):
                
                purchase_request_id = purchase_request_obj.search([('mrp_id', '=', x.raw_material_production_id.id)])

                if purchase_request_id: 

                    purchase_request_line_id = purchase_request_line_obj.search([('product_id', '=', x.product_id.id), ('request_id', 'in', purchase_request_id.ids)])
                ...
                vals={'company_id': company_id,
                    ...
                    'name': name,
                    'product_list_qty': x.product_uom_qty - x.quantity_available,
        ...
        if mrp_bom_id:
            ...
            for x in self.move_raw_ids:
                ...
                vals={'company_id': company_id,
                    'product_id': x.product_id.id,
                    'name': name,
                    'product_list_qty': x.product_uom_qty,
                    'product_qty_actual': x.product_id.qty_available,
                    'product_qty_sent': x.product_id.virtual_available,
                    'product_uom_id':x.product_id.uom_id.id,
                    'date_required': date_current,
